...
def process_helix(company, helix_id, api_key):
    ...
    def get_search_saved():
        ...
        """
        try:
            url = f"{base_url}/helix/id/{helix_id}/api/v3/search/saved/"
            headers = {
    ...
    def run_search(search):
        ...
        month_name = months_br.get(month_number, 'Desconhecido')

        save_path = os.path.join('dados', str(year), company, month_name)
        ...
        if not os.path.exists(filename):
            try:
                ...
                elif 'OrigenFailureLogin' in name or 'Origen Failure Login' in name: 
                    process_archive(company, name, origin_failure_login(company, name))
                else:
    ...
    def process_archive(company,name, query):
        ...
        def get_date_range(query):
            ...
            if c_day == 1:
                ...
                search_end_date = f'{c_year}-{c_month}-{c_day}T23:59:59Z'
            elif c_day == 30:
                search_start_date = f'{c_year}-{c_month}-01T00:00:00Z'
                search_end_date = f'{c_year}-{c_month}-{c_day}T{c_hour-4}:59:59Z'    
            else:
                search_start_date = f'{c_year}-{c_month-1}-01T00:00:00Z'
                search_end_date = f'{c_year}-{c_month}-{c_day}T{c_hour-3}:59:59Z'
...
def delete_all(configs):
    ...
    for config in configs:
        ...
        if helix_id and api_key:
            ...
            # else:
            #     logging.info(f"{company} - Nenhuma pesquisa arquivada para deletar.")
        else:
...
# EXTRAS
def origin_failure_login(company, name):
    ...
    filename = os.path.join(save_path, "FailureLogin.xlsx")
    ...
    query = f'targetusername={user} | groupby srcipv4'
